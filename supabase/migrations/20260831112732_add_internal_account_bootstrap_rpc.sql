...
begin
  ...
  select true, (u.email_confirmed_at is not null)
    into v_user_exists, v_email_confirmed
  from auth.users u
  where u.id = p_user_id;

  if not found then
    v_valid_until_epoch_millis := (extract(epoch from (now() + interval '5 minutes')) * 1000)::bigint;
  ...
  select a.verified_age_tier, a.app_role, a.account_state
    into v_age_tier, v_app_role, v_account_state
  ...
  v_profile_readiness := case
    ...
    when coalesce(length(btrim(v_username)), 0) < 3 then 'INCOMPLETE'
    else 'READY'
  end;
  ...
  if not v_access_exists or v_account_state <> 'active' or v_age_tier = 'unknown' then
    ...
    if v_age_tier = 'under_16' then
      v_restrictions := array_append(v_restrictions, 'PROTECTED_CONTENT_ONLY');
    ...
    elsif not v_caps_exists or v_cap_age_tier <> v_age_tier then
      v_restrictions := array_append(v_restrictions, 'DMS_RESTRICTED');
...
revoke execute on function public.internal_account_bootstrap(uuid) from public;
revoke execute on function public.internal_account_bootstrap(uuid) from anon;
revoke execute on function public.internal_account_bootstrap(uuid) from authenticated;
grant execute on function public.internal_account_bootstrap(uuid) to service_role;

comment on function public.internal_account_bootstrap(uuid) is
  'Server-only projection of trusted account state for the Patsy account-bootstrap Edge Function. Client roles cannot execute it directly.';
